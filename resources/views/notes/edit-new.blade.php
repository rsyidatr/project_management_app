<x-minimal-layout>
        ...
        <div class="flex-1 flex flex-col bg-white">
            ...
            <div class="border-b border-gray-100 px-6 py-5">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <a href="{{ route('notes.show', $note) }}" class="p-2 hover:bg-gray-100 rounded-lg transition-colors duration-200">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-gray-500 mt-1">{{ $note->title }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex-1 p-6 bg-gray-50 overflow-y-auto">
                <div class="max-w-3xl mx-auto">
                    <form action="{{ route('notes.update', $note) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-gray-100">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="p-8 space-y-6">
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent text-lg font-medium @error('title') border-red-300 @enderror"
                                >
                                @error('title')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select 
                                        name="type" 
                                        class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('type') border-red-300 @enderror"
                                    >
                                        <option value="note" {{ old('type', $note->type) == 'note' ? 'selected' : '' }}>📝 Note</option>
                                        <option value="project" {{ old('type', $note->type) == 'project' ? 'selected' : '' }}>📋 Project</option>
                                        <option value="meeting" {{ old('type', $note->type) == 'meeting' ? 'selected' : '' }}>👥 Meeting</option>
                                        <option value="goal" {{ old('type', $note->type) == 'goal' ? 'selected' : '' }}>🎯 Goal</option>
                                        <option value="recipe" {{ old('type', $note->type) == 'recipe' ? 'selected' : '' }}>🍳 Recipe</option>
                                    </select>
                                    @error('type')
                                    <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                                    @enderror
                                ...
                                <!-- Folder -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-3">Folder</label>
                                    <select 
                                        name="folder_id" 
                                        class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('folder_id') border-red-300 @enderror"
                                    >
                            ...
                            <div>
                                ...
                                <input 
                                    type="text" 
                                    name="tags" 
                                    value="{{ old('tags', implode(', ', $note->tags_array ?? [])) }}" 
                                    placeholder="Enter tags separated by commas..."
                                    class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('tags') border-red-300 @enderror"
                                >
                                <p class="text-xs text-gray-500 mt-2">Separate tags with commas (e.g., work, important, meeting)</p>
                                @error('tags')
                                <p class="text-red-600 text-sm mt-2">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none @error('content') border-red-300 @enderror"
                        ...
                        <div class="px-8 py-6 bg-gray-50 border-t border-gray-100 rounded-b-xl">
                            <div class="flex items-center justify-between">
                                <div class="text-sm text-gray-500">
                                    <kbd class="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-xs">Ctrl</kbd> + 
                                    <kbd class="px-2 py-1 bg-gray-100 border border-gray-200 rounded text-xs">S</kbd> to save
                                </div>
                                <div class="flex items-center gap-3">
                                    <a href="{{ route('notes.show', $note) }}" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors duration-200 font-medium">
                                        Cancel
                                    </a>
                                    <button type="submit" class="px-6 py-3 bg-gray-900 text-white rounded-lg hover:bg-gray-800 transition-colors duration-200 font-medium shadow-sm">
                                        Update Note
                                    </button>
                                </div>
                            </div>
    ...
    <!-- Create Note Modal (for Quick Note) -->
    <div id="createNoteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center p-4 z-50">
        <div class="bg-white rounded-xl max-w-lg w-full p-6 shadow-2xl">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-lg font-semibold text-gray-900">Create Quick Note</h3>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('keydown', function(e) {
                // Ctrl/Cmd + S to save (prevent default and submit form)
                if ((e.ctrlKey || e.metaKey) && e.key === 's') {
                    e.preventDefault();
                    const form = document.querySelector('form[action*="notes.update"]');
                    if (form) {
                        form.submit();
                    }
                }
                
                // Ctrl/Cmd + N to create new note (open modal)
                if ((e.ctrlKey || e.metaKey) && e.key === 'n') {
                    ...
                    if (createNoteModal) {
                        ...
                        setTimeout(() => {
                            const titleInput = createNoteModal.querySelector('input[name="title"]');
